...
class ContactUsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String? getSocialLink(SocialType type) {
      if (companyData?.socials == null) return null;
      for (final s in companyData!.socials) {
        if (s.socialType == type) return s.link;
      }
      return null;
    }

    final contactEmail = getSocialLink(SocialType.contactEmail) ?? "support@z-ecommerce.com";
    final contactPhone = getSocialLink(SocialType.contactPhoneFirst) ?? "+1 800 123 4567";
              ...
              child: Column(
                ...
                children: [
                  ...
                  _ContactItem(
                    ...
                    value: companyData?.addresses?.firstOrNull?.address.get(context) ??
                        "123 Ecommerce St, City, Country",
                    onTap: () async {
                      final addr = companyData?.addresses?.firstOrNull;
                      if (addr != null) {
